...
const decisionWarnings: Record<ReviewDecision, { color: string; message: string } | null> = {
  ...
  legal: {
    color: "warn",
    message: "Routing to Legal Queue. This action requires department head sign-off.",
  },
  procurement: { color: "warn", message: "Routing to Procurement for pricing and terms review." },
  "high-risk": {
    color: "error",
    message: "High risk — escalating to executive review immediately.",
  },
              ...
              {warn && (
                <div
                  className={`mt-4 border-l-2 p-3 rounded-r-md flex items-start gap-2 ${
                    warn.color === "error" ? "bg-error-container border-error" : "bg-warn-bg"
                  }`}
